...
the data in `iris` is numerical, we will add in two additional made-up variables
...
head(iris) %>% kable(escape=F, align="cl") %>% trimws %>% kable_styling(c("striped","bordered"))
```
...
input the variable's label as the rowlabel.
```{r}
info <- info %>% num_row(row_var="Sepal.Length", rowlabels="Sepal Length")
info[[7]] %>% kable(escape=F, align="l") %>% trimws %>% kable_styling(c("striped","bordered"))
```
...
info[[8]] %>% indent(5) %>% kable(escape=F, align="l") %>% trimws %>% kable_styling(c("striped","bordered"))
```
...
info[[9]] %>% indent(5) %>% kable(escape=F, align="l") %>% trimws %>% kable_styling(c("striped","bordered"))
```
...
```{r, eval=FALSE}
info <- tbl_start(iris, NULL, missing=FALSE, overall=TRUE, comparison=FALSE) %>%
  num_row("Sepal.Length", rowlabels="Sepal Length") %>%
  tbl_out() %>%
  print()
```
...
info <- tbl_start(iris, NULL, missing=FALSE, overall=TRUE, comparison=FALSE) %>%
  num_row("Sepal.Length", rowlabels="Sepal Length") %>%
  tbl_out()
  info[[8]] %>% kable(escape=F, align="l") %>% trimws %>% kable_styling(c("striped","bordered"))
```
...
```
```{r}
head(birds) %>% kable(escape=F, align="l") %>% trimws %>% kable_styling(c("striped","bordered"))
```
...
info <- tbl_start(iris, "Species", missing=FALSE, overall=TRUE, comparison=FALSE) %>%
  ...
  cat_row(row_var="Region", col_var="Type", newdata=birds) %>%
  tbl_out()
  info[[14]] %>% indent(5) %>% kable(escape=F, align="l") %>% trimws %>% kable_styling(c("striped","bordered"))
```
