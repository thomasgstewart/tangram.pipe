...
num_default <- function(dt, rowlabel, missing, digits){
  rnd <- paste0("%.", digits, "f")
  if (!is.null(ncol(dt))){
    if (missing == TRUE){
    ...
    out$Overall <- ""
    out$Overall[1] <- sprintf(rnd, min(dt))
    out$Overall[2] <- sprintf(rnd, as.numeric(quantile(dt,.25)))
    out$Overall[3] <- sprintf(rnd, median(dt))
    out$Overall[4] <- sprintf(rnd, as.numeric(quantile(dt,.75)))
